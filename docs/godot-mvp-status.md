...
- 2026-05-21 Route Risk / Reward Density / QA Readiness 020-022 已分段落地：`020-022-route-risk-reward-density-qa-readiness` 拆成三個 commit。020 補 route risk 明細與事件風險緩衝；`playable_demo_auto_run_tests.gd` / `multiseed_balance_probe_tests.gd` 的 structured log 現在除 `route_risk_summary` 外，也輸出 `route_risk_events`，逐筆記錄 event id、floor、option label、HP / Gold / curse / relic delta、是否觸發事件戰鬥與 risk tags。`RandomMapGenerator` 會在近期高風險事件後優先抽低風險事件，避免高失血 / curse / Gold 花費連續疊加，但不移除高風險事件、不調敵人整體壓力。021 補 reward path density telemetry；log 現在有 `reward_choice_summaries`、`deck_archetype_timeline`、`key_pickup_floors`，multiseed summary 聚合 `key_pickup_floor_totals` 與 `reward_choice_count`，可判斷失敗 run 是缺 bridge、defense、payoff，還是 reward 有給但 proxy 沒選。021 gameplay 只做 AZKi late payoff reward sorting 小調整，floor >= 10 且缺 Laplus payoff 時更積極排序 `azki-necrobinder-finale`、`azki-laplus-combo`、`azki-laplus-release`、`azki-laplus-overflow`；Subaru 嘗試過 pre-elite 防守排序但會讓 `subaru_midrun_hp_pressure_watch` 退化，因此未保留；Botan 沒有主動 buff。022 收斂 QA readiness：結算畫面仍維持短版 `QA 回報摘要`，不直接顯示 `route_risk_events`、`reward_choice_summaries`、`deck_archetype_timeline`、`key_pickup_floors` 等長 telemetry key；完整資料留在 structured `qa_report` / headless log。最新 multiseed 觀測為 Subaru 7/10、Botan 8/10、AZKi 8/10 抵達 `boss_reward`；AZKi 死於 `ssrb-giant-camouflage` 2 次、Subaru 未出現 `ssrb-debuff-check` 重複死亡，符合 guardrail。watch flags 仍有 `azki_boss_pacing_watch` 1 次、`subaru_midrun_hp_pressure_watch` 3 次、`event_risk_compounding_watch` 1 次；本輪未做 GUI/manual QA。
- 2026-05-21 Targeted Pacing / Balance / QA Readiness 017-019 已分段落地：`017-019-targeted-pacing-balance-qa-readiness` 拆成三個 commit。017 補 headless pacing telemetry，不改 gameplay；`playable_demo_auto_run_tests.gd` 與 `multiseed_balance_probe_tests.gd` 現在除 `energy_summary` 外，也輸出 `combat_summaries`、`boss_pacing_summary`、`route_risk_summary`、`run_health_flags`，並把 `boss_pacing_summary` / `route_risk_summary` / `run_health_flags` 保留在 structured `qa_report`，不塞進結算畫面長文。018 做小範圍平衡：AZKi 只補 late payoff（`azki-laplus-combo` 10x2 -> 11x2、`azki-necrobinder-finale` 22+10 -> 24+12 / 升級 26+12 -> 28+14），Subaru 只補中段防守 / payoff 安全性（`subaru-encore-recall` block 4/7 -> 6/9、`subaru-afterimage-table` 條件 block 5/7 -> 6/9），並微調 reward sorting 讓 AZKi late 缺收束、Subaru 中段缺防守時更容易看到對應 bridge / payoff；Botan 沒有主動 buff。019 強化 headless UI guard 與文件：長事件 / chapter start / run_end 文字仍需保持在 `BASE_SIZE` 內，結算畫面不直接顯示 telemetry key，文件 guard 要求 `boss_pacing_summary`、`route_risk_summary`、`run_health_flags` 等欄位被記錄。018 後 multiseed 觀測為 Subaru 7/10、Botan 7/10、AZKi 7/10 抵達 `boss_reward`；AZKi 死於 `ssrb-giant-camouflage` 2 次、Subaru 死於 `ssrb-debuff-check` 1 次，符合 guardrail。watch flags 仍有 `azki_boss_pacing_watch` 1 次、`subaru_midrun_hp_pressure_watch` 4 次、`event_risk_compounding_watch` 1 次，下一輪 manual QA 優先看 AZKi boss turns、Subaru floor 7-11 低血壓力、事件失血 / curse / Gold 花費疊加。
- 2026-05-21 Energy Telemetry / Subaru Economy Audit 016 已落地：從 015 merged 後的 `main` 開 `016-energy-telemetry-subaru-economy-audit`，只補 headless telemetry 與 schema guard，不調卡牌、不調敵人、不改 reward 權重、不新增 gameplay 數值變更。`playable_demo_auto_run_tests.gd` 與 `multiseed_balance_probe_tests.gd` 的 combat resolver 現在每個玩家回合記錄 `turn_start_energy`、`energy_spent`、`energy_gained`、`turn_end_energy`、`cards_played`，並彙總到 `energy_summary`：`combat_count`、`turns_total`、`average_unspent_energy`、`max_turn_end_energy`、`high_unspent_energy_turns`、`energy_spent`、`energy_gained`、`cards_played`、`economy_flags`。Subaru 若 `average_unspent_energy >= 1.5` 且 `high_unspent_energy_turns >= 8`，只會在 log 標記 `subaru_energy_overflow_watch`，不讓測試失敗，避免未手測前過早 nerf。固定 multiseed 仍為 Subaru 6/10、Botan 7/10、AZKi 7/10 抵達 `boss_reward`；本輪觀測 Subaru `average_unspent_energy` 約 0.15、`high_unspent_energy_turns` 為 8，未觸發 overflow watch。這表示目前 headless proxy 沒看到系統性「能量花不完」，但仍保留手測對照入口。
- 2026-05-21 AZKi Boss 節奏 / Subaru 中段穩定度 015 已落地：從本機 merge 014 後的 `main` 開 `015-azki-boss-pacing-subaru-midrun`，只做 headless 可驗證的小範圍平衡，不做 GUI/manual QA、不調 Botan、不改敵人整體壓力、不新增 runtime 系統。AZKi 補 boss 收束效率：`azki-coordinate-barrage` 2x3 -> 3x3、`azki-laplus-release` 12 + 4 -> 14 + 6 / 升級 14 + 6 -> 16 + 8、`azki-laplus-overflow` base 12 -> 14 / 升級 16 -> 18。Subaru 補中段防守但不加能量：`subaru-rhythm-guard` block 7 -> 9、`subaru-crowd-cover` block 7 -> 9 / 升級 10 -> 12、`subaru-new-oshi-call` block 4 -> 6。新增 fixture 鎖 AZKi formed deck 對 `ssrb-giant-camouflage` 與 Subaru natural midrun 對 `ssrb-debuff-check`，並在 multiseed failure analysis 加 guard：AZKi 死於 `ssrb-giant-camouflage` 不可超過 2 次、Subaru 死於 `ssrb-debuff-check` 不可超過 1 次。multiseed 結果為 Subaru 6/10、Botan 7/10、AZKi 7/10 抵達 `boss_reward`；Subaru `ssrb-debuff-check` 重複死亡從 2 次降為 1 次，AZKi `ssrb-giant-camouflage` 維持 2 次內。
- 2026-05-21 Multiseed Failure Analysis 014 已落地：從 013 merged / pushed 後的 `main` 開 `014-multiseed-failure-analysis`，不調整卡牌、敵人、reward 或 combat runtime。`multiseed_balance_probe_tests.gd` 在 013 的 `multiseed_balance_probe_failure_cases` 之外，新增 `multiseed_balance_probe_failure_analysis`，依角色聚合 `repeated_defeat_floors`、`repeated_defeat_enemy_ids`、`boss_failures`、`midrun_failures`、`curse_seen` 與 `likely_issues`，讓後續可先看 repeated pattern 再決定是否需要調平衡。這輪仍不做 GUI/manual QA，也不自動把分析結果轉成 buff / nerf。
- 2026-05-21 QA Report Failure Triage 013 進行中：從 012 merged / pushed 後的 `main` 開 `013-qa-report-failure-triage`，不調整卡牌、敵人、reward 或 combat runtime。`multiseed_balance_probe_tests.gd` 現在除逐筆 `multiseed_balance_probe_log` 與角色 summary 外，額外輸出 `multiseed_balance_probe_failure_cases`，把每個失敗 seed 的 character、seed、boss、defeat_floor、defeat_enemy_id、defeat_hp、中文 deck/relic summary 與可複製 `qa_report_text` 集中列出；角色 summary 也帶 `failure_cases`，讓後續不用在 30 筆 log 內人工撈重複死亡樓層 / 敵人 / 牌組模式。這輪目標是改善 012 後的手測 / headless 交接效率，仍不做 GUI/manual QA。
- 2026-05-20 Manual QA UI / Laplus / AZKi Polish 012 已落地：從 011 merged 後的 `main` 開 `012-manual-qa-ui-polish-laplus-azki`，處理使用者手測回報但不代做 GUI/manual QA。事件與 Chapter start event 會顯示 HP / Gold / Deck / Relic 狀態列，長文與選項改用固定區塊換行；結算畫面 `QA 回報摘要` 改為截圖友善欄位，主要 deck/relic 以中文名稱摘要顯示，完整 `deck_ids` / `relic_ids` 仍留在結構化 `qa_report`。戰鬥手牌 hover 不再放大原本按鈕 hitbox，改建立 `CombatHoverPreview` 非互動放大層，降低 enter/exit 抖動與點擊需要多次的問題。AZKi 的 Laplus 若在敵方回合倒下，下一個玩家回合開始會以 1 HP 復活；活著時不自動疊 HP。AZKi 節奏小幅補強：`azki-map-shot` 8 -> 10、`azki-pinpoint` 5 -> 7、`azki-laplus-dash` 7 -> 10，只改善擊殺效率，不調 Botan 或敵人壓力。新增 / 更新 headless UI overflow guard、manual QA reporting、combat engine 與 runtime database 測試；本輪仍不做 GUI/manual QA。
- 2026-05-20 Auto-run Report Parity 011 已落地：從 010 merged 後的 `main` 開 `011-auto-run-report-parity`，不調整卡牌、敵人、reward 或 combat runtime。`Game.gd` 將 010 的結算畫面 `QA 回報摘要` 抽成 `_manual_qa_report_snapshot()` 結構化資料，再由 `_manual_qa_report_text_from_snapshot()` 轉成畫面文字；manual run_end、`playable_demo_auto_run_tests.gd` 與 `multiseed_balance_probe_tests.gd` 現在共用同一套欄位。auto-run / multiseed JSON log 會輸出 `qa_report` 與 `qa_report_text`，方便直接比較玩家回報與 headless proxy。新增測試要求 snapshot 必須包含 character、result、seed、boss、defeat floor/enemy/hp、deck_ids、relic_ids，並要求 auto-run / multiseed log 不可缺 QA report。
- 2026-05-20 Manual QA Reporting Support 010 已落地：從 009 merged 後的 `main` 開 `010-manual-qa-reporting-support`，不改 combat runtime、不調角色 / 敵人 / reward 平衡，也不做 GUI/manual QA。結算畫面 `run_end` 現在會顯示可直接抄回報的 `QA 回報摘要`，包含角色、結果、seed、Boss、死亡樓層、死亡敵人、死亡前 HP、Deck、Relic、體感問題、UI 問題。新增 `manual_qa_reporting_tests.gd` 以 headless 方式驗證死亡結算會帶出這些欄位；`balance-playtest-prep.md` 同步記錄 010 後使用者只需補體感與 UI 問題，不必手動整理 deck/relic snapshot。
...
- UI：
  ...
  - 戰鬥手牌未 hover 時以畫面中央為核心，維持中間較高、左右較低的底部扇形排列；手牌數量減少時也會往中央集中。012 起 hover 不再改變原卡牌按鈕 hitbox，而是在上方建立 `CombatHoverPreview` 非互動放大層，原卡維持固定位置與尺寸以穩定點擊；preview 底部貼齊畫面下緣並用實際尺寸重排文字，降低文字模糊並保留較完整描述區比例。
  - 下方手牌區不再顯示「手牌」標題文字。
...
- 本輪 UI / 行為修正：
  ...
  - 2026-05-20 AZKi / Laplus 召喚物規則微調：`CombatState` 保留 `summon_*` 狀態，AZKi 戰鬥開始建立 Laplus `1` HP；敵方攻擊先被玩家格擋擋住，剩餘傷害由 Laplus 攔截，overflow 才扣 AZKi HP。012 起 Laplus 若在敵方回合倒下，下一個玩家回合開始會以 `1` HP 復活；若 Laplus 還活著，回合開始不自動補 HP、不疊 HP，仍需透過卡牌 `summon_heal` 往上疊。`Game.gd` 同時顯示 `AZKiBodySprite`、`LaplusSummonSprite` 與必要時的 `PlayerActionFxSprite`，Laplus 頭上顯示目前 summon HP 或 `DOWN`。
